...
export function listings(version: string): Listing[] {
  const app = appListing(document, {
    ...
    long_description: [
      ...
      "",
      "A guild admin presses Connect once and installs the app at GitHub, choosing the account and which repositories it may see on GitHub's own page. Nothing is typed here and nobody pastes a token — so the issue count, the last fortnight's activity and the open Dependabot alerts are there for everybody. Members who want their own review queue connect their own GitHub account; nobody else has to.",
      "",
