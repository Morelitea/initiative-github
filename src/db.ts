...
export const SCHEMA: SchemaStep[] = [
  ...
  {
    name: "0002-webhook-delivery-ledger",
    risk: "additive",
    statements: [
      `CREATE TABLE IF NOT EXISTS webhook_deliveries (
         ...
         CONSTRAINT webhook_deliveries_state_shape CHECK (
      ...
      `CREATE INDEX IF NOT EXISTS webhook_deliveries_completed_at
         ON webhook_deliveries (completed_at) WHERE state = 'completed'`,
    ],
  },
];
 ...
 *
 * The first insert preserves `seen_at` for the lifetime of the row. A retry may
 * replace an expired lease, but never the row or that audit timestamp.
 *
 * Completed rows are retained long enough to suppress GitHub's redelivery
 * window. Processing rows are never swept: an expired one is reclaimed in
 * place so a crashed process cannot turn into a permanent refusal.
 */
export type DeliveryStart = "started" | "duplicate" | "in_progress";

export async function beginDelivery(
  deliveryId: string,
  leaseToken: string
): Promise<DeliveryStart> {
  const client = await pool.connect();
  try {
    await client.query(
    ...
    const claimed = await client.query(
      `INSERT INTO webhook_deliveries
         (delivery_id, state, lease_token, lease_until)
       VALUES ($1, 'processing', $2, now() + ($3 * interval '1 second'))
       ON CONFLICT (delivery_id) DO UPDATE
         SET state = 'processing',
             lease_token = EXCLUDED.lease_token,
             lease_until = EXCLUDED.lease_until,
             completed_at = NULL
       WHERE webhook_deliveries.state = 'processing'
         AND webhook_deliveries.lease_until <= now()
       RETURNING delivery_id`,
      [deliveryId, leaseToken, DELIVERY_LEASE_SECONDS]
    );
    if (claimed.rowCount === 1) return "started";

    const existing = await client.query<{ state: "processing" | "completed" }>(
      "SELECT state FROM webhook_deliveries WHERE delivery_id = $1",
      [deliveryId]
    );
    return existing.rows[0]?.state === "completed" ? "duplicate" : "in_progress";
  } finally {
    client.release();
  }
}

async function completeDelivery(deliveryId: string, leaseToken: string): Promise<void> {
  ...
  if (completed.rowCount !== 1) {
    throw new Error("delivery processing lease expired before completion");
  }
}
...
export type DeliveryRun<T> =
  | { kind: "processed"; result: T }
  | { kind: "duplicate" }
  | { kind: "in_progress" };

/** Run work once after completion, while allowing failed attempts to retry. */
export async function runDeliveryOnce<T>(
  deliveryId: string,
  work: () => Promise<T>
): Promise<DeliveryRun<T>> {
  const leaseToken = randomUUID();
  const start = await beginDelivery(deliveryId, leaseToken);
  ...
  try {
    ...
    try {
      await releaseDelivery(deliveryId, leaseToken);
    } catch (releaseError) {
